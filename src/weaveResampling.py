...
def resampleCont( w , R=0 ):
    ...
    u = ( ( np.arange(0,R,1) + float( np.random.uniform() ) ) / float(R) ).astype(float);
...
def resampleMultinomial(w, N=0 ):
    ...
    weave.inline(code,['u','H','ww','Ind','N'], type_converters=weave.converters.blitz)
    return Ind;
